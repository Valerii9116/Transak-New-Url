...
const TransakWidget = () => {
  ...
  useEffect(() => {
    // Define all widget initialization functions inside useEffect to avoid dependency issues
  ...
  // Standalone function for manual retry
  const retryConnection = async () => {
    try {
      setIsLoading(true);
      setError('');
      
      const authResponse = await fetch('/api/transak/auth', {
        method: 'POST',
      ...
      const widgetResponse = await fetch('/api/transak/create-widget-url', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
      ...
      if (!widgetResponse.ok) throw new Error('Failed to create widget URL');

      const widgetData = await widgetResponse.json();
      const finalUrl = widgetData.url || widgetData.widgetUrl;
  ...
  // ✅ FIXED: This function was not properly defined
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-8">
      <div className="max-w-7xl mx-auto px-4">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Transak Widget Integration</h1>
          <p className="text-gray-600 text-lg">Production-ready crypto trading with secure API integration</p>
        </div>
        ...
        {!error && widgetUrl && (
          <div className="mb-6 bg-green-50 border border-green-200 rounded-lg p-4">
            <div className="flex items-start">
              <div className="flex-shrink-0"><span className="text-green-400 text-xl">✅</span></div>
              <div className="ml-3">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            {/* Amount */}
            <div className="mb-4">
                <label className="block text-sm font-medium text-gray-700 mb-2">Default Amount ({config.fiatCurrency})</label>
                <input
                  type="number"
            ...
            <div className="space-y-3">
                <label className="flex items-center">
                  <input type="checkbox" checked={config.hideMenu} onChange={(e) => handleConfigChange('hideMenu', e.target.checked)} className="mr-2" />
                  <span className="text-sm">Hide Menu</span>
                </label>
                <label className="flex items-center">
                  <input type="checkbox" checked={config.isAutoFillUserData} onChange={(e) => handleConfigChange('isAutoFillUserData', e.target.checked)} className="mr-2" />
                  <span className="text-sm">Auto-fill User Data</span>
                </label>
                <label className="flex items-center">
                  <input type="checkbox" checked={config.disableWalletAddressForm} onChange={(e) => handleConfigChange('disableWalletAddressForm', e.target.checked)} className="mr-2" />
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            {/* Remaining JSX for preview panel */}
             <div className="mt-6 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg">
                <h3 className="text-sm font-semibold text-gray-800 mb-3">Current Configuration:</h3>
              ...
              <div className="mt-2 text-center">
                <p className="text-xs text-gray-500">
                  {widgetUrl ? 'Connected to live Transak API' : 'API connection required for widget'}
                </p>
              </div>
          </div>
        </div>
